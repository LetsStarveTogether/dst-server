...
`Server.start()` 默认用 900 秒覆盖进程创建、原生就绪确认和首次 Lua driver 安装。
...
SDK 按操作类型提供默认值，普通调用无需手动传入 `timeout`。
Python runtime、Agent、Controller、RPC client 和 Cap'n Proto 参数采用相同的默认值。

| 操作 | 默认时间 |
| --- | --- |
| 普通命令和类型化游戏请求 | 120 秒 |
| 保存并等待确认 | 300 秒 |
| 重置、回档、按天回档、重新生成世界 | 900 秒 |
| 单次游戏启动及 Lua driver 安装 | 900 秒 |
| 游戏进程正常停止 | 120 秒；强制退出及输出清理另计 |
| `connect()` 连接及握手 | 60 秒 |

集群保存、重置和回档在取得操作锁并确认当前就绪后开始计时。
这份预算覆盖事件标记、主分片执行、全部分片确认，以及按天回档的快照选择和结果核验。
重载确认包含世界重新加载和新一代 Lua driver 就绪；保存确认包含命令写入和 Saved 事件。
嵌套调用和新建的请求任务继承同一个截止时间，不会在阶段切换时重置预算。
超时后已提交的操作可能仍在游戏中继续执行，SDK 会报告不确定结果。

RPC 请求另留 60 秒用于控制器预检、转发及响应传输。
无显式超时参数的启动、重启和 Mod 更新请求默认最多等待三小时，以覆盖 Mod 准备和启动重试。
停止请求默认最多等待 300 秒，强制停止请求为 120 秒；事件订阅的 `next()` 保持长轮询。
生成的 Quadlet 为容器停止保留 360 秒，systemd 停止保留 420 秒，避免截断 SDK 的退出清理。
Netdata 日志查询默认 120 秒，包含并发槽位等待和子进程执行，超时后清理查询进程。
...
`ClusterClient.rollback_to_day(day, timeout=900)` 返回实际选中的 `Snapshot`。
