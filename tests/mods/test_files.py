...
def test_native_scan_accepts_static_calls_inside_native_setup(tmp_path: Path) -> None:
    ...
    assert mods.scan_setup(setup) == ((7, 8, 42), (99,))
...
def test_strict_setup_rejects_dynamic_or_unsupported_code(
    ...
    with pytest.raises(ValueError, match=r"Workshop|literal|return"):
...
async def test_log_handler_failure_terminates_process_group(
    ...
    processes = (0, 0)
    monkeypatch.setattr(mod_process, "TERMINATE_GRACE_PERIOD", 0.01)
    ...
    with pytest.raises(RuntimeError, match="log failed"):
        await mods.update_native(executable, ugc, log_handler=fail)
...
async def test_cancellation_terminates_process_group(
    ...
    task = asyncio.create_task(
        mods.update_native(executable, ugc, log_handler=handle_log)
    )
...
def test_static_setup_uses_configuration_model(tmp_path: Path, source: str) -> None:
    setup = tmp_path / "setup.lua"
    setup.write_text(source, encoding="utf-8")
...
def test_native_scan_is_broader_than_static_configuration(
