...
async def test_close_reclaims_the_pending_result_reader() -> None:
    existing_tasks = asyncio.all_tasks()
    console, writer, _ = make_console()
    executing, _, _, _ = await start_command(console, writer, "pending")
    reader = console.pending_result
    ...
    try:
        ...
        executing.cancel()
        reader.cancel()
        await asyncio.gather(executing, reader, return_exceptions=True)


@pytest.mark.parametrize("cancellations", [1, 3])
async def test_close_finishes_writer_before_propagating_repeated_cancellation(
...
@pytest.mark.parametrize("payload", [b"", b"partial", b"partial\n", b"x" * 131072])
async def test_incomplete_response_breaks_console_before_next_write(
    payload: bytes,
) -> None:
    console, writer, reader = make_console()
    first, _, _, _ = await start_command(console, writer, "first")
