...
def make_pipeline(
    ...
    delivery = importlib.import_module("dst_server.telemetry.outbox")
...
async def test_invalid_rejection_count_is_quarantined_as_protocol_failure(
    tmp_path: Path,
) -> None:
    sender = Sender(
        ExportLogsServiceResponse(
    ...
    try:
        ...
        assert len(sender.requests) == 1
        assert pipeline.status().pending == 0
        assert pipeline.status().last_error == "invalid_export_response"
    finally:
        await pipeline.shutdown()
...
@pytest.mark.parametrize("cancel_count", [1, 3])
async def test_cancelled_shutdown_waits_for_admitted_writes_and_releases_storage(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, cancel_count: int
) -> None:
    sender = Sender()
    sender.failure = grpc.StatusCode.UNAVAILABLE
    ...
    def blocked_append(payload: bytes) -> int:
        ...
        if not release.wait(3):
            message = "test writer was not released"
            raise TimeoutError(message)
        return append(payload)

    monkeypatch.setattr(pipeline.outbox, "append", blocked_append)
    writing = asyncio.create_task(pipeline.emit_event(observed()))
    await until(entered.is_set)
    closing = asyncio.create_task(pipeline.shutdown())
    try:
        await asyncio.sleep(0)
...
async def test_provider_cleanup_continues_after_one_shutdown_failure(
    ...
    pipeline.tracer_provider = first
    pipeline.meter_provider = second
...
async def main():
    ...
    another = otel.configure(outbox_path=Path(sys.argv[1]))
    assert another.tracer_provider is not pipeline.tracer_provider
    assert another.meter_provider is not pipeline.meter_provider
    assert trace.get_tracer_provider() is pipeline.tracer_provider
    assert metrics.get_meter_provider() is pipeline.meter_provider
