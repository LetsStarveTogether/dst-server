...
from dst_server.deployment.models import UnitField
from dst_server.timeouts import RPC_TIMEOUT_MARGIN


@pytest.mark.parametrize("model", [PodUnit, ContainerUnit])
def test_quadlet_native_keys_cover_each_serialized_field_once(
    model: type[PodUnit | ContainerUnit],
) -> None:
    keys = []
    for name, field in model.model_fields.items():
        if name == "name":
            continue
        metadata = [item for item in field.metadata if isinstance(item, UnitField)]
        assert len(metadata) == 1, name
        keys.append((metadata[0].section, metadata[0].key))
