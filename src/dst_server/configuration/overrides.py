...
from luaparser.astnodes import Call, Name, Return, String
...
SETUP_FUNCTIONS = {"ServerModSetup": 0, "ServerModCollectionSetup": 1}
...
class CustomWorldOverrides(WorldOverrides):
    __pydantic_extra__: dict[
        Annotated[str, AfterValidator(validate_lua_string)],
        LuaScalar,
    ] = Field(init=False)
...
class _WorldPayload[Kind, Overrides](RevalidatedFrozenModel):
    kind: Kind
    values: Overrides


type _TaggedWorld = Annotated[
    _WorldPayload[Literal["world"], WorldOverrides]
    | _WorldPayload[Literal["forest"], ForestOverrides]
    | _WorldPayload[Literal["cave"], CaveOverrides]
    | _WorldPayload[Literal["quagmire"], QuagmireOverrides]
    | _WorldPayload[Literal["lavaarena"], LavaArenaOverrides]
    | _WorldPayload[Literal["custom"], CustomWorldOverrides]
    | _WorldPayload[Literal["loaded_forest"], _LoadedForestOverrides]
    | _WorldPayload[Literal["loaded_cave"], _LoadedCaveOverrides],
    Field(discriminator="kind"),
]
_WORLD_JSON = TypeAdapter(_TaggedWorld)
_WORLD_KINDS = {
...
def _decode_world(value: object, info: ValidationInfo) -> object:
    if info.mode == "json":
        return _WORLD_JSON.validate_python(value).values
    return value


def _encode_world(
    value: WorldOverrides,
    handler: SerializerFunctionWrapHandler,
    _info: SerializationInfo,
) -> dict[str, object]:
    return {
        "kind": _WORLD_KINDS.get(type(value), "custom"),
        "values": handler(value),
    }


type _WorldValue = Annotated[
    SerializeAsAny[WorldOverrides],
    BeforeValidator(_decode_world, json_schema_input_type=_TaggedWorld),
    WrapSerializer(_encode_world, when_used="json"),
    GetPydanticSchema(
        get_pydantic_json_schema=lambda _schema, handler: handler(
            _WORLD_JSON.core_schema
        ),
    ),
]
...
class LevelDataOverride(RevalidatedFrozenModel):
    ...
    def load(
        ...
        *,
        ...
        from .files import load_lua_table

        values = load_lua_table(path, "DST level data override configuration")
    ...
    def render(self) -> str:
        ...
        return f"return {render_literal(values)}\n"
...
class WorldgenOverride(RevalidatedFrozenModel):
    ...
    overrides: _WorldValue = Field(default_factory=WorldOverrides)
    ...
    def load(  # ruff: ignore[complex-structure, too-many-branches]
        ...
        *,
        ...
        values = _upgrade_worldgen_v1(
            load_lua_table(path, "DST world override configuration")
        )
    ...
    def render(self) -> str:
        validated = type(self).model_validate(self)
        overrides = render_literal(_validated_world_values(validated.overrides), 1)
        lines = [
            "return {",
...
class ModSettings(RevalidatedFrozenModel):
    ...
    @classmethod
    def load(cls, path: Path) -> Self:
        from .files import load_lua_calls
        ...
        values: dict[str, object] = {}
        for function, arguments in load_lua_calls(
            path,
    ...
    def load(cls, path: Path) -> Self:
        ...
        for function, arguments in load_lua_calls(
            ...
            if not isinstance(identifier, str):
                msg = f"{function} argument must be a literal string"
                raise ValueError(msg)  # ruff: ignore[type-check-without-type-error]
...
def scan_setup(path: Path) -> tuple[tuple[int, ...], tuple[int, ...]]:
    ...
    if not path.is_file():
        return (), ()
    values: tuple[set[int], set[int]] = (set(), set())
    for statement in parse_statements(read_text(path), "DST Mod setup configuration"):
        ...
        for call in calls or ():
            ...
            if not value.isascii() or (not value and index == 0):
                continue
            if not value.isdecimal() or value.startswith("0"):
                msg = f"invalid Workshop ID: {value!r}"
                raise ValueError(msg)
            item = int(value)
            if item > MAX_WORKSHOP_ID:
                msg = f"Workshop ID exceeds uint64: {item}"
                raise ValueError(msg)
...
def has_setup_code(path: Path) -> bool:
    ...
    return path.is_file() and bool(
        parse_statements(read_text(path), "DST Mod setup configuration")
    )
